...
public:
	...
	static bool FindNearstIntersection(Ray& ray, Scene& scene, Real t_min, Real t_max);
	static bool FindNearstIntersection_AABB(Ray& ray, Scene& scene, std::vector<AxisAlignedBoundingBox>& boxs);
	static bool FindNearstIntersection_BVH(Ray& ray, Scene& scene, BVH& node, Real t_min, Real t_max);
private:
