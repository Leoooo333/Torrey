...
bool BVH::isHit(Ray& ray, double t_min, double t_max, Scene& scene, pcg32_state& rng)
{
    ...
    if (left_shape) // not nullptr
    {
        ...
        bool hit_left = ray.FindIntersection(*left_shape, t_min, t_max, scene, rng);
        if(hit_left)
        {
            t_max = ray.distance;
        }
        ...
        hit_right = ray.FindIntersection(*right_shape, t_min, t_max, scene, rng);
        //else
        //    hit_right = false;
    ...
    {
        bool hit_left = left->isHit(ray, t_min, t_max, scene, rng);
        bool hit_right = right->isHit(ray, t_min, hit_left ? ray.distance: t_max, scene, rng);
        return hit_left || hit_right;
    }
    //return true;
}
