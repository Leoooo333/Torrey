			...
			if (scene.src_shapes[area_light.shape_id]->index() == 0) // sphere
			{
				...
				for (int i = 0; i < sqrt_N; i++) {
					for (int j = 0; j < sqrt_N; j++) {
						...
						Real cos_theta_max = sqrt(1. - (light_sphere.radius * light_sphere.radius) / dc * dc);
						//std::cout << "cos_max:" << cos_theta_max << std::endl;
						Real theta = acos(1. + (cos_theta_max - 1.) * u1);
						//Real theta = acos(1. - 2. * u1);
						Real phi = 2. * c_PI * u2;
						//Real ds = dc * cos(theta) - sqrt(light_sphere.radius * light_sphere.radius - dc * dc * sin(theta) * sin(theta));
						//Real cos_alpha = (dc * dc + light_sphere.radius * light_sphere.radius - ds * ds) / (2 * dc * light_sphere.radius);
